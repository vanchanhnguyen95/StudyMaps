    ...
    public class RegionManagerV2
    {
                                ...
                                for (int j = 0; j < objectCount; j++)   
                                {
                                    ...
                                    if (RunningParams.HTProvincePriority.ContainsKey(province.ProvinceID) == false)
                                    {
                                        ...
                                        if(RunningParams.HTProvincePriority.ContainsKey(province.ProvinceID) == true)
                                            LogFile.WriteFileV2("", "ProvincePriority", "", province.ProvinceID.ToString());
                                    }
                                    ...
                                    for (int k = 0; k < length; k++)
                                        province.PointList.Add(new BAGPointV2(BitConverter.ToDouble(reader.ReadBytes(8), 0), BitConverter.ToDouble(reader.ReadBytes(8), 0)));
                                    //Add object
        ...
        private static bool CheckMore(short provinceID)
        {
            try
            {
                ...
                {
                    LogFile.WriteProcessV2("ProvinceRoadByLevel: " + RunningParams.ProvinceRoadByLevel);
            ...
            {
                LogFile.WriteErrorV2("RegionManager.CheckMore, ex: " + ex.ToString());
                return false;
            }
        }
    }

...
namespace BAGeocoding.Dal.MapObj
{
    ...
    /// </summary>
    public class TileDAOV2 : NoSqlHelper
    {
        private static IMongoCollection<BAGTileV2> _collection;

        public TileDAOV2()
        {
            ConventionRegistry.Register("elementNameConvention", pack, x => true);
            _collection = database.GetCollection<BAGTileV2>(nameof(BAGTileV2));
        }

        public static async Task<List<BAGTileV2>> GetAll()
        {
            try
            {
                var ds = await _collection.Find(x => x.TileID == (short)EnumBAGRegionType.Tile).ToListAsync();
                if (ds != null)
                    return ds;
        ...
        public static List<BAGTileV2> GetByPage()
        {
            int pageNumber = 0;
            int totalCount = 0;
            ...
            try
            {
                List<BAGTileV2> tileList = new List<BAGTileV2>();
                do
                {
                    List<BAGTileV2> tileTemp = GetByPage(pageNumber++, ref totalCount);
                    if (tileTemp == null)
                        break;
        ...
        private static List<BAGTileV2> GetByPage(int pageNumber, ref int totalCount)
        {
            try
            {
        ...
        /// </summary>
        public static async Task<bool> Add(BAGTileV2 tile)
        {
            try
            {
                tile.LngStr = tile.LngStr ?? string.Empty;
                tile.LatStr = tile.LatStr ?? string.Empty;
        ...
        public static async Task<bool> Clear()
        {
            try
            {
                var filter = Builders<BAGTileV2>.Filter.Eq(c => c.TileID, (short)EnumBAGRegionType.Tile);

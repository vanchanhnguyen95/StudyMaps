...
namespace BAGeocoding.Api.Controllers.V1
{
    ...
    public class RoadNameController : BaseController
    {
        ...
        public RoadNameController(IRoadNameService roadNameService, IVietNamShapeService vietNamShapeService)
        {
        ...
        public async Task<IActionResult> AddDataVietNamShape(string path = @"D:\Project\Els01\Province.json")
        {

            if (string.IsNullOrEmpty(path)) path = @"D:\Project\Els01\Province.json";
            var jsonData = System.IO.File.ReadAllText(path);
            var roadPushs = JsonConvert.DeserializeObject<List<VietNamShape>>(jsonData);

            return Ok(await _vietNamShapeService.BulkAsync(roadPushs ?? new List<VietNamShape>()));
        }
